...
import { useOrderMutation } from "@/components/hook/orderHooks";
...
function PlaceOrder() {
  ...
  const { mutateAsync: createOrder, isLoading } = useOrderMutation();
  ...
  useEffect(() => {
    if (!cart.paymentMethod || !userInfo) {
      router.push("/");
    }
